...
data class Board(
    ...
    fun getSquareAt(col: Int, row: Int): Square? {
        if (row >= 8 || row < 0 || col >= 8 || col < 0) {
            Log.e(TAG, "There is no square at col $col and row $row")
            return null
        }
    ...
    fun movePiece(move : MoveMessage) : Board {
        ...
        if(!isValidMove(startSquare, endSquare)){
            Log.e(TAG, "Move is invalid. Can't play it.")
            return this
        }
    ...
    fun isCastlingMove(start: Square, end: Square): Boolean {
        ...
        Log.i(TAG,"move from square: " + start + "to " + end + "is a castling move")
        return true
    }
    fun isValidMove(start: Square, end: Square): Boolean {
        ...
        if (!canPieceMoveTo(movingPiece, end)) {
            Log.e(TAG,"Move is invalid because the piece can't move to there")
            return false
        }
    ...
    fun canPlayerPlay(playerColor: Color): Boolean {
        Log.i(TAG, "checking if player: " + playerColor + "can play")
        for (row in 0..7) {
            for (col in 0..7) {
    ...
    private fun doesPieceHaveAMove(piece: Piece, startSquare: Square): Boolean {
        for (targetSquare in piece.getMovableSquares(this)) {
            if (isValidMove(startSquare, targetSquare)) {
                Log.e(TAG,"The piece on square: $startSquare  can move to the square: $targetSquare")
                return true
            }
        }
        return false
